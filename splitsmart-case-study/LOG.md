...
- **2026-05-28 — Budget page (Layer 2) built.** Direction A+C hybrid: cap meter hero (£524 of £700, avg/day vs cap-allows) + side-by-side donuts "By group" (Paris 67% / Home Bills 27% / Work Lunch 6%) and "By category" (composition only, no caps). No per-category caps (friction + you don't control shared-expense categories), no forecast (preachy), only SplitSmart group-share spend (no rent/solo). Group numbers are this-month-share only since groups span months. Budget added as 4th sidebar item across all 11 pages.
- **2026-05-28 — UI audit pass + beautification (Phase A+B).** Ran 5-agent UI audit then 3-agent beautification audit. Shipped: red+yellow fixes (drop direction-encoding minuses, reduce coral overload, settle-up width/chrome, dashboard layout, donut palette, H1 bumps, CTA verb standardization, balance math reconciled to Mina +£30/Luigi −£20/net +£10), plus visual polish (layered shadows, body aurora, sidebar deep gradient + active glow, hero glint on dashboard balance card, glossy gradient CTAs, tinted settle-up bodies, scales→avatar swap, voice rewrites). Removed dashboard Insights+Settled+Recent-activity-across-groups cards; trimmed Spending-over-time to half-width.
